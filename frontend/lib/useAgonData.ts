...
async function fetchAgon(client: PublicClient): Promise<AgonData> {
  ...
  return { stats, match, leaderboard };
}

/** Live on-chain data when contracts are configured; otherwise mock data. */
export function useAgonData(): AgonData & { isLive: boolean } {
  ...
  if (!isConfigured || !query.data) {
    return { stats: mockStats, match: mockMatch, leaderboard: mockLeaderboard, isLive: false };
  }
